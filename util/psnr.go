...
// - 100 dB means that new has the same quality than ref
// - 0 dB means that you do not compare the same image.
func PSNR(ref, new hdr.Image) (mse, snr, psnr, peak float64) {
	d := ref.Bounds()
	var signal float64
	var noise float64
	...
	for y := 0; y < d.Dy(); y++ {
		for x := 0; x < d.Dx(); x++ {
			x1, y1, z1, _ := ref.HDRAt(x, y).HDRXYZA()
			x2, y2, z2, _ := new.HDRAt(x, y).HDRXYZA()
